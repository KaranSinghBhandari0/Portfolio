...
export default function Skills() {
  const techStack = [
    ...
    {
      name: "GTM",
      img: "https://cdn.simpleicons.org/googletagmanager",
    },
    {
      name: "Vercel",
  ...
  return (
    <div
      className="bg-[#000000] bg-[radial-gradient(#ffffff33_1px,#00091d_1px)] bg-[size:20px_20px] min-h-screen pt-16 "
      id="skills"
    >
        ...
        <div className="w-full max-w-2xl grid grid-cols-[repeat(auto-fit,minmax(80px,1fr))] gap-x-6 gap-y-8 mx-auto">
          {techStack.map((tech, i) => (
            <div
              title={tech.name}
              key={i}
              className="w-20 h-20 bg-gradient-to-br from-[#1e1e1e] to-[#2a2a2a] rounded-lg shadow-lg flex justify-center items-center 
                    hover:rounded-[50%] hover:scale-110 hover:shadow-[0px_0px_15px_#00ffff] transition-all duration-500 ease-in-out"
            >
              <img
                src={tech.img}
                alt={tech.name}
                className="w-12 h-12 transition-all duration-500 ease-in-out hover:scale-125"
              />
            </div>
          ))}
        </div>
